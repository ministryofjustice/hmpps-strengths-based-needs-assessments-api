...
SAN_SECRET_NAME="rds-postgresql-instance-output"
SAN_LOCAL_PORT=5435
...
kubectl -n "${SAN_NAMESPACE}" port-forward pod/${POD_NAME} ${SAN_LOCAL_PORT}:5432 >/tmp/portforward-sp.log 2>&1 &
SAN_PF_PID=$!
...
echo "Cleaning up AAP data..."

psql ${AAP_CONNECTION_STRING} -f ./cleanup/aap.sql

echo "Cleaning up SAN data..."

psql ${SAN_CONNECTION_STRING} -f ./cleanup/sp.sql

echo "Cleaning up Coordinator data..."

psql ${COORDINATOR_CONNECTION_STRING} -f ./cleanup/coordinator.sql
