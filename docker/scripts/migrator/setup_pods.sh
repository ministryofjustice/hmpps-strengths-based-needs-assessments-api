...
kubectl -n "${AAP_NAMESPACE}" delete pod --ignore-not-found=true "${POD_NAME}"
...
kubectl -n "${AAP_NAMESPACE}" run "${POD_NAME}" \
  --image="${POD_IMAGE}" \
  --port=5432 \
  --env="REMOTE_HOST=${AAP_INSTANCE_ADDRESS}" \
  --env="LOCAL_PORT=5432" \
  --env="REMOTE_PORT=5432"
...
kubectl -n "${AAP_NAMESPACE}" wait --for=condition=Ready pod/${POD_NAME} --timeout=60s
