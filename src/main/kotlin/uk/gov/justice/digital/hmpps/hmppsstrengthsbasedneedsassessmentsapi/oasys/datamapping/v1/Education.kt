...
class Education : SectionMapping() {
  ...
  private fun getSeverityOf(field: Field): Int? = when (ap.answer(field).value) {
  ...
  private fun q9(): Any? = when (ap.answer(Field.EDUCATION_HIGHEST_LEVEL_COMPLETED).value) {
    ap.get(Value.LEVEL_1), ap.get(Value.ENTRY_LEVEL), ap.get(Value.NONE_OF_THESE) -> when (
      ap.answer(
  ...
  private fun q94(): Any? = PractitionerAnalysis("EMPLOYMENT_EDUCATION", ap).notes()

  private fun q96(): Any? = PractitionerAnalysis("EMPLOYMENT_EDUCATION", ap).riskOfSeriousHarm()

  private fun q98(): Any? = PractitionerAnalysis("EMPLOYMENT_EDUCATION", ap).riskOfReoffending()

  private fun qStrength(): Any? = PractitionerAnalysis("EMPLOYMENT_EDUCATION", ap).strengthsOrProtectiveFactors()
