...
class Relationships : SectionMapping() {
  ...
  private fun q8(): Any? =
    if (ap.answer(Field.LIVING_WITH).values?.contains(ap.get(Value.PARTNER)) == true) "1"
    else if (ap.answer(Field.PERSONAL_RELATIONSHIPS_COMMUNITY_IMPORTANT_PEOPLE).values?.contains(ap.get(Value.PARTNER_INTIMATE_RELATIONSHIP)) == true) "2"
    else "3"
  ...
  private fun q97(): Any? = PractitionerAnalysis("PERSONAL_RELATIONSHIPS_COMMUNITY", ap).notes()

  private fun q98(): Any? = PractitionerAnalysis("PERSONAL_RELATIONSHIPS_COMMUNITY", ap).riskOfSeriousHarm()

  private fun q99(): Any? = PractitionerAnalysis("PERSONAL_RELATIONSHIPS_COMMUNITY", ap).riskOfReoffending()

  private fun qStrength(): Any? = PractitionerAnalysis("PERSONAL_RELATIONSHIPS_COMMUNITY", ap).strengthsOrProtectiveFactors()
}
