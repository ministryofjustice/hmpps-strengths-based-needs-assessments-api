...
data class AssessmentVersionCriteria(
  ...
  fun getSpecification(): Specification<AssessmentVersion> = belongsToAssessment()
    .and(hasTag())
    .and(isVersionNumber())
    .and(versionNumberFrom())
    .and(versionNumberTo())

  private fun belongsToAssessment(): Specification<AssessmentVersion> = Specification<AssessmentVersion> { root, _, builder ->
    builder.equal(root.get(AssessmentVersion_.assessment).get(Assessment_.uuid), assessmentUuid)
  }

  private fun hasTag(): Specification<AssessmentVersion> = Specification<AssessmentVersion> { root, _, builder ->
    tags?.takeIf { it.isNotEmpty() }?.let { builder.and(root.get(AssessmentVersion_.tag).`in`(tags)) }
  }

  private fun isVersionNumber(): Specification<AssessmentVersion> = Specification<AssessmentVersion> { root, _, builder ->
    versionNumber?.let {
      builder.equal(root.get(AssessmentVersion_.versionNumber), it)
    }
  }

  private fun versionNumberFrom(): Specification<AssessmentVersion> = Specification<AssessmentVersion> { root, _, builder ->
    versionNumberFrom?.let {
      builder.greaterThanOrEqualTo(root.get(AssessmentVersion_.versionNumber), it)
    }
  }

  private fun versionNumberTo(): Specification<AssessmentVersion> = Specification<AssessmentVersion> { root, _, builder ->
    versionNumberTo?.let {
